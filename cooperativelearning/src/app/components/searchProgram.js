...
export const SearchProgram = () => {
  ...
  const ChoosingPrograms = ["All", ...new Set(programname.map((program) => program.programname))];
...
if(loading){
  ...
  return (
    <div className="search-program">
          ...
          {filterPrograms.map((program) => (
            ...
            >
              ...
              >
                  ...
                  <h2 className="text-xl font-semibold text-gray-800 mb-2 group-hover:text-blue-600 transition-colors">{program.programname}</h2>
      ...
      <p>Showing {filterPrograms.length} of {programname.length}</p>
    </div>
  );
};